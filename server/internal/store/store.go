...
func (s *Store) CreateLicense(ctx context.Context, label, keyHash, keyPrefix string, productID, policyID uuid.UUID, expiresAt *time.Time, createdBy *uuid.UUID) (*License, error) {
	const q = `
		INSERT INTO licenses (label, key_hash, key_prefix, product_id, policy_id, expires_at, created_by)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id
	`

	var id uuid.UUID
	if err := s.pool.QueryRow(ctx, q, label, keyHash, keyPrefix, productID, policyID, expiresAt, createdBy).Scan(&id); err != nil {
