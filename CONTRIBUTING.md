...
Publishing triggers `.github/workflows/release.yml`, which builds binaries and container images via GoReleaser and publishes the Helm chart in a separate workflow job. The release job stamps `docs/openapi.yaml` `info.version` from the git tag (strip leading `v`) and attaches the stamped copy to the GitHub release. The stamp is publish-time only — git keeps the `0.0.0-dev` placeholder, and the workflow marks the file skip-worktree so GoReleaser still sees a clean worktree. GoReleaser also generates a changelog from git commits as a fallback for empty GitHub release bodies; with `release.mode: keep-existing`, Release Drafter notes on the published release are left unchanged.

Each server release also ships SPDX JSON SBOMs (Syft): one per linux archive (generated by GoReleaser, checksumed in `checksums.txt`) and one per GHCR image platform (`linux/amd64`, `linux/arm64`, uploaded after the image is published). Local `make release` requires [Syft](https://github.com/anchore/syft) on your `PATH`.

Published GHCR images are signed with keyless Cosign and include a GitHub Artifact Attestation with SLSA build provenance. See [docs/deployment.md](docs/deployment.md#releases) for verification commands.
