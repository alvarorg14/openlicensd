...
| `config load failed` + `must be a boolean` / `must be an integer` | Unparsable boolean or integer env value (e.g. `OPENLICENSD_LOCAL_LOGIN_ENABLED=no`, or `OPENLICENSD_REQUEST_BODY_MAX_BYTES` set to scientific notation like `1.048576e+06`) | Use `true`/`false`/`1`/`0` for booleans and a base-10 integer for numeric vars; see [configuration.md](configuration.md). Helm chart v1.0.0+ renders integer ConfigMap keys as decimal strings — upgrade the chart if you see scientific notation in the pod env |
