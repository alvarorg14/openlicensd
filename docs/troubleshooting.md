...
| `config load failed` + `must be a boolean` / `must be an integer` | Unparsable boolean or integer env value (e.g. `OPENLICENSD_LOCAL_LOGIN_ENABLED=no`) | Use `true`/`false`/`1`/`0` for booleans and a base-10 integer for numeric vars; see [configuration.md](configuration.md) |
