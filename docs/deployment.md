...
OpenLicensd publishes **Linux amd64 and arm64** server artifacts. GoReleaser ([`.goreleaser.yaml`](../.goreleaser.yaml)) builds `linux/amd64` and `linux/arm64` release archives only — there are no published darwin or Windows binaries.
