...
- **Content**: `docs/*.md` served in place; `README.md`, `QUICKSTART.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `COMPATIBILITY.md`, and `SUPPORT.md` included via VitePress file includes (no duplication)
...
- `values.schema.json` rejects unknown Helm values keys (`additionalProperties: false` on defined objects), so removed keys such as `secret.data.databaseUrl` fail at install time instead of being silently ignored; `global` is the umbrella-chart exception (accepted but unused by templates)
...
| `OPENLICENSD_RATE_LIMIT_ENABLED` | `true` | Enable rate limiting on public, login, and authenticated endpoints |
...
| `COMPATIBILITY.md` | API stability, SemVer scope, and deprecation policy for `/api/v1` (from v1.0.0) |
...
make test-ui       # Playwright E2E tests (requires Postgres, build, Chromium)
...
Server releases attach SPDX JSON SBOMs generated by [Syft](https://github.com/anchore/syft):

| Trigger | Purpose |
|---------|---------|
| GoReleaser `sboms` pipe (`.goreleaser.yaml`) | SPDX SBOM per linux archive; included in `checksums.txt` |
| Release workflow post-publish step | SPDX SBOM per GHCR image platform (`linux/amd64`, `linux/arm64`) uploaded to the GitHub release |
| CI GoReleaser snapshot (`.github/workflows/ci.yml`) | Verifies archive SBOM generation on every PR |

Syft is pinned in workflow files; Renovate proposes updates. Local `make release` requires Syft on `PATH`.

### Image signing and provenance (Cosign)

Server releases sign the published GHCR multi-arch image digest with keyless [Cosign](https://github.com/sigstore/cosign) and attach a GitHub Artifact Attestation with SLSA build provenance via [`actions/attest`](https://github.com/actions/attest):

| Trigger | Purpose |
|---------|---------|
| Release workflow post-publish step (`.github/workflows/release.yml`) | `cosign sign` on the GHCR index digest; `actions/attest@v4` with `push-to-registry: true` |

Cosign is pinned in the release workflow; Renovate proposes updates. Verification commands are documented in [docs/deployment.md](docs/deployment.md#releases) and [README.md](README.md).
